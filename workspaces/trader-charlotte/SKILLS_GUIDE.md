...
> 本角色可挂自己的技能（skill）：把程序性、可触发、带脚本 / 素材的能力封装起来。本指南讲怎么用、怎么写；装载机制的底层细节见 `memory/agent-skill-playbook.md`，撰写规范见 `memory/skill-authoring-playbook.md`。
...
技能分两层放：本角色专属的放 `workspaces/european-session-trader/skills/`；所有角色共用的放共用层。本指南聚焦本工作区的 `skills/`。
...
> 完整规范与 grep 命令见 `memory/skill-authoring-playbook.md`。
