...
- **⚠️ `api` 透传信封判成功看 `res.code===0`，不是 `res.ok`**：`larkExec(['api', METHOD, path, ...])` 这种**原生透传**调用，返回信封用 `code`（0=成功）+ `data`（=接口的 `data` 字段，如 `res.data.items`/`res.data.node`）；`res.ok` 是 `+command` 封装才有的字段，透传调用**可能没有**。写健壮判定：`res && (res.ok === true || res.code !== 0 ? … )`——即 `res.ok===true || res.code===0` 才算成功（`getMessageById`/建 wiki 节点都这么判）。搞错会把成功当失败。
