...
| 活动 / Meetup 模块（@ 或自然语言预约飞书会议 + 视频、循环默认×10、标签、按标签订阅、08:00 围观群播报 @ 订阅者、每次增删改刷【社区行事历】wiki；日历 / scope / 分享链接踩坑） | `activity-meetup-playbook.md` |
...
- **数据库 schema**：已到 **v25**（启动自动迁移；各版本含义见对应 playbook）。v21=`chat_reactions` 表（表情反应采集、per-member 累计点赞数、驱动【点赞狂魔】里程碑），见 `community-notify-events-playbook.md §11`；v22=`pinned_messages` 表（**热门消息自动置顶**：某消息当天被去重 ≥N 人反应就 `im pins` 置顶、幂等、配置驱动 `autoPinMinReactors`、独立于事件系统），见 `community-notify-events-playbook.md §12`；**v23/v24/v25=活动 / Meetup 模块**（`activity_meetups` + `activity_meetup_tags` + `meetup_subscriptions` + `share_link` 列，per-soul `tudigong.db`），见 `activity-meetup-playbook.md`；**v26=`visitor_milestones`**（访客里程碑防重发台账：重启不失效、只发一次、记第 100·N 位访客+回填+wiki），见 `community-notify-events-playbook.md §14`。**手动带自定义 vars 触发任一事件**（如 `first-try-notify`【某成员首次玩新功能】发围观群+LP+10）：反查名字→open_id 用 `store.findOpenIdsByName`、`AGENT_SOUL=<soul> node --env-file=.env` 跑临时 `fireEvent(id,{actorOpenId,vars})`（CLI 传不了 vars）、发奖走 `afterSend`+`grantPt`、`unsend` 撤回不退 LP、profile/账本在 `.agent/shared.db` 表 `pt_ledger`，见 `community-notify-events-playbook.md §13`。
