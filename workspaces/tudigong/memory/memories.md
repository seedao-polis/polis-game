...
- **数据库 schema**：已到 **v22**（启动自动迁移；各版本含义见对应 playbook）。v21=`chat_reactions` 表（表情反应采集、per-member 累计点赞数、驱动【点赞狂魔】里程碑），见 `community-notify-events-playbook.md §11`；v22=`pinned_messages` 表（**热门消息自动置顶**：某消息当天被去重 ≥N 人反应就 `im pins` 置顶、幂等、配置驱动 `autoPinMinReactors`、独立于事件系统），见 `community-notify-events-playbook.md §12`。
