<overview>
Skills improve through iteration and testing. This reference covers evaluation-driven development, the agent A/B testing patterns, and XML structure validation during testing.
</overview>
...
<principle>
The most effective skill development uses the agent itself. Work with "the agent A" (expert who helps refine) to create skills used by "the agent B" (agent executing tasks).
</principle>

<creating_skills>
<workflow>
<step_1>
**Complete task without skill**: Work through problem with the agent A, noting what context you repeatedly provide.
</step_1>

<step_2>
**Ask the agent A to create skill**: "Create a skill that captures this pattern we just used"
</step_2>
...
<step_5>
**Test with the agent B**: Use fresh instance to test on real tasks.
</step_5>

<step_6>
**Iterate based on observation**: Return to the agent A with specific issues observed.
</step_6>
</workflow>

<insight>
The agent models understand skill format natively. Simply ask the agent to create a skill and it will generate properly structured SKILL.md content.
</insight>
</creating_skills>
...
<workflow>
<step_1>
**Use skill in real workflows**: Give the agent B actual tasks.
</step_1>
...
<step_3>
**Return to the agent A**: Share observations and current SKILL.md.
</step_3>

<step_4>
**Review suggestions**: The agent A might suggest reorganization, stronger language, or workflow restructuring.
</step_4>
...
<haiku_testing>
**The agent Haiku** (fast, economical)
...
<opus_testing>
**The agent Opus** (powerful reasoning)
...
1. Start fresh conversation (the agent B)
