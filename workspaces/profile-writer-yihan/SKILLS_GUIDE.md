...
> 本角色可挂自己的技能（skill）：把程序性、可触发、带脚本 / 素材的能力封装起来。本指南讲怎么用、怎么写；装载机制与撰写规范的完整说明，见共用 skill `create-agent-skills`。
...
技能分两层放：本角色专属的放 `workspaces/profile-writer-yihan/skills/`；所有角色共用的放共用层 `workspaces/_shared/skills/`。本指南聚焦本工作区的 `skills/`。
...
> 完整规范、骨架模板与 grep 自检命令，见共用 skill `workspaces/_shared/skills/create-agent-skills/`。
