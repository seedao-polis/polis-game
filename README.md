...
- **回复尾注显示的是整回合净变化**：`🌱 LP : <前> → <后> (<净变化>)` 由框架（非 LLM）追加，数字**从账本 `pt_ledger` 按本回合的 `ref_message_id` 求和读回**，所以 agent 在回合中自己调 `pt_grant` 工具改的 LP 也会被算进去，不会只显示框架自己那两笔。首次见面礼 +120 不计入（新人首轮尾注仍是 `120.0 → 119.9 (-0.1)`）。
- 详见 `workspaces/tudigong/memory/pt-gamification-playbook.md` 第 4.1（尾注净变化）、9（共享库）、10（评分机制）节。
...
`tools/` 下放**独立于本框架**的 Rust 工具，各有自己的 `Cargo.toml`/`.env`，不进 tsc / pnpm workspace，不整合进 agent 的 pm2，只通过文件路径共享同一颗 SQLite 库。

- **`tools/seedaodb/`**：给 tudigong 的 SQLite 库（`.agent/tudigong.db`、`.agent/shared.db`）开一个带鉴权的 HTTP SQL CRUD 网关（axum + sqlx）。三道闸门：服务令牌 → 飞书 / 静态身份 → 按用户的 RBAC（数据来源级读 / 写）。⚠️ 写 `shared`（LP 账本）会绕过 `grantPt` 记账，默认把它设为只读保护。
- **`tools/seedaodb-client/`**：上面服务的 Rust CLI 客户端，含飞书 OAuth 登录与权限呈现；操作看它自己的 `README.md`。

各工具自带 `README.md` 与 `.env.example`；配置文件（`.env`、`config/*.toml`）已 gitignore，用前先从 `*.example` 复制。完整设计与踩坑见 `workspaces/tudigong/memory/seedaodb-tool-playbook.md`。
